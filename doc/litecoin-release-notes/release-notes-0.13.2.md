Breekcoin Core version 0.13.2 is now available from:

  <https://download.breekcoin.org/breekcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a breekcoin
...
ECDSA signatures inside Breekcoin transactions now use validation using
...
Previous versions of Breekcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Breekcoin Core was to raise the default minimum
relay fee.

Breekcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Breekcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running breekcoind without having to do any manual configuration.
...
Breekcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Breekcoin Core will relay transactions with
...
entirely in Breekcoin Core 0.13, and it is at this time undecided whether the
...
Breekcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Breekcoin Core automatically creates a hidden service to listen on, without
manual configuration. Breekcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Breekcoin Core is listening, and
...
Breekcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Breekcoin
Core to use floating fees. By default, Breekcoin Core will use floating
fees.
...
(default: `0.0002` BRKC/kB).

At all times, Breekcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BRKC.
Furthermore, Breekcoin Core will never create transactions paying less than
...
`breekcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 21802 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 21802 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /breekcoinrpc>
        ProxyPass http://127.0.0.1:1802/
        ProxyPassReverse http://127.0.0.1:1802/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass breekcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< breekcoinrpc:<password>
    </Location>
...
Restarting breekcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in breekcoin.conf
...
breekcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/breekcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Breekcoin Core code base has
...
- `breekcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `breekcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Breekcoin scripting system that in combination with
...
Breekcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Breekcoin Core.
...
The two phases are distinct in the Breekcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Breekcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Breekcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Breekcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of breekcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of breekcoins being spent is exactly
...
- **Increased security for multisig:** Breekcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Breekcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Breekcoin's current ledger (called the UTXO set),
  ...
  note that Breekcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Breekcoin users to individually opt-in to almost any change in the Breekcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Breekcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Breekcoin
client to Breekcoin Core 0.13.2 to signal for segwit support. When a
...
Since Breekcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `breekcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `breekcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
