...
const char * const BITCOIN_CONF_FILENAME = "breekcoin.conf";
...
#else
    const char* pszModule = "breekcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Breekcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Breekcoin";
#else
    // Unix
    return pathRet / ".breekcoin";
#endif
#endif
}
